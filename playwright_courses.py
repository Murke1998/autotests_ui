...
with sync_playwright() as playwright:
    ...
    courses_empty_state_icon = page.get_by_test_id('courses-list-empty-view-icon')
    expect(courses_empty_state_icon).to_be_visible()

    courses_no_results_text = page.get_by_test_id('courses-list-empty-view-title-text')
    expect(courses_no_results_text).to_be_visible()
    expect(courses_no_results_text).to_have_text("There is no results")

    courses_empty_state_description = page.get_by_test_id('courses-list-empty-view-description-text')
    expect(courses_empty_state_description).to_be_visible()
    expect(courses_empty_state_description).to_have_text("Results from the load test pipeline will be displayed here")